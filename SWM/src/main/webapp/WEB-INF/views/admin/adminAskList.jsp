...
<body>
  ...
  <h1 data-v-7c8cb348="">1:1 문의 리스트</h1>     
		...
		<hr style="background:#d6d6d6; height:1px;">
				      ...
				      <div data-v-7c8cb348 class="list">
				      		<c:choose>
							  <c:when test="${fn:length(allAsk) > 0}">
								 <c:forEach items="${allAsk}" var="allAsk">
				      
				      
							   	<a data-v-7c8cb348 href="/admin/askReply?ask_no=${allAsk.ask_no }" class="item">
					
					         	<h2 data-v-7c8cb348="">${allAsk.ask_title }</h2>
					         	<p data-v-7c8cb348 class="info" >
					        	 	<span data-v-7c8cb348="">작성자 : ${allAsk.nickName }</span>
					         		<span data-v-7c8cb348="">${allAsk.ask_date }</span>
					         		<c:choose>
					         		<c:when test="${reply == 0 }">
